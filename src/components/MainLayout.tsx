...
import { Home, DollarSign, ListChecks, Wallet, BarChart, Settings, LogOut, Menu } from 'lucide-react';
...
import AppLogo from './AppLogo'; // Import AppLogo
...
const NavLink: React.FC<NavLinkProps> = ({ to, icon, label, isActive, onClick }) => (
  <Link to={to} onClick={onClick} className={cn(
    ...
    isActive && "bg-gradient-to-r from-emerald-green to-teal-blue text-white hover:from-emerald-green/90 hover:to-teal-blue/90"
  )}>
    {icon}
    {label}
  </Link>
  ...
  const SidebarContent = ({ onClose }: { onClose?: () => void }) => (
    <div className="flex h-full max-h-screen flex-col gap-4 p-4">
      <div className="flex h-14 items-center border-b px-4 lg:h-[60px] lg:px-6">
        <AppLogo to="/" showText={true} animated={true} />
      </div>
    ...
    <div className="grid min-h-screen w-full md:grid-cols-[220px_1fr] lg:grid-cols-[280px_1fr]">
      ...
      {isMobile && (
        <header className="flex h-14 items-center gap-4 border-b bg-gray-100 dark:bg-gray-900 px-4 lg:h-[60px] lg:px-6 md:hidden">
          ...
          <AppLogo to="/" showText={true} animated={false} /> {/* No animation on mobile header logo */}
        </header>
      )}
